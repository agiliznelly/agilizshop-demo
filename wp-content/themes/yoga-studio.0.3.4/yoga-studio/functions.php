...
function yoga_studio_notice(){
    ...
    if ( is_admin() && ('themes.php' == $pagenow) && isset( $_GET['activated'] ) && $theme_name->get( 'TextDomain' ) === 'yoga-studio' ) {
...
function yoga_studio_scripts() {
	...
	// Block Style
	wp_enqueue_style( 'yoga-studio-block-style', esc_url( get_template_directory_uri() ).'/assets/css/blocks.css' );
...
function yoga_studio_fonts_scripts() {
	$headings_font = esc_html(get_theme_mod('yoga_studio_headings_text'));
	$body_font = esc_html(get_theme_mod('yoga_studio_body_text'));

	if( $headings_font ) {
		wp_enqueue_style( 'yoga-studio-headings-fonts', '//fonts.googleapis.com/css?family='. $headings_font );
	} else {
		wp_enqueue_style( 'yoga-studio-source-sans', '//fonts.googleapis.com/css?family=Lato:400,700,400italic,700italic');
	}
	if( $body_font ) {
		wp_enqueue_style( 'yoga-studio-body-fonts', '//fonts.googleapis.com/css?family='. $body_font );
	} else {
